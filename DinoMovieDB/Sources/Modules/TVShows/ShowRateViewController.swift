...
class ShowRateViewController: UIViewController {
    ...
    init(showId: Int, showTitle: String) {
        self.showId = showId
        self.showTitle = showTitle
        viewModel = RateItemViewModel(itemId: showId, itemName: showTitle)
    ...
    private func setupViews() {
        title = R.string.localization.tv_show_rate_title()
    ...
    private func setupBindings() {
        ...
        ratePublisher.sink(onReceived: rateShow).store(in: &cancellables)
    ...
    private func rateShow() {
        itemsStateService.rate(showId: showId, rating: viewModel.ratingValue)
            .map { _ in }
    ...
    private func deleteRating() {
        itemsStateService.deleteRateOn(showId: showId)
            .map { _ in }
    ...
    private func fetchState() {
        itemsStateService.fetchTvShowsState(showId: showId)
            .map(\.rating)
